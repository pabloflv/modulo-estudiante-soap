...
@Data
public class AlumnosMateriaExcelExporter {

    private AlumnosMateriaDTO alumnosMateria;

    private XSSFWorkbook workbook;

    private XSSFSheet sheet;

    public AlumnosMateriaExcelExporter(AlumnosMateriaDTO alumnos) {
        this.alumnosMateria = alumnos;
        workbook = new XSSFWorkbook();
        sheet = workbook.createSheet("Cursada " + alumnos.getMateria().getNombre());
    }
    ...
    private void writeHeaderRow() throws DecoderException {
        ...
        cell.getSheet().addMergedRegion(new CellRangeAddress(0, 0, 0, 4));
        ...
        cell.setCellValue(
                "Crusada " + alumnosMateria.getMateria().getNombre() + " - " + alumnosMateria
                        .getMateria()
                        .getId());

        Row rowData = sheet.createRow(1);
        String[] columns =
                {"Codigo usuario", "Nombre y apellido", "DNI", "Nota TP", "Nota parcial"};
    ...
    private void writeDataRows() {
        ...
        int numRow = 2;
        for (AlumnoMateriaDTO alumno : alumnosMateria.getAlumnos()) {
            ...
            Cell tpCell = rowDataValue.createCell(3);
            tpCell.setCellStyle(cellStyle);

            Cell examCell = rowDataValue.createCell(4);
